#### Main script for project with Guga on folded opinion, EXP6
#### In this version we avoid latitude of acceptance,
#### diversified memorizing of info and diversified folding point.
#### Now we test effect of average communication contacts in comm. network.

## Encoding: windows-1250
## Created:  2023-06-29 Fran»esko
## Edited:   2023-08-01 Fran»esko
...
experiment =
  function(opDistributionX = "Black Pete", rounds = 100, Nx = 1000, seedX = 1,
           ...
           foldingX = foldingPoint,  neis_sdX = neis_sd,
  ...
  agm = createPublic(N = Nx, opDistribution = opDistributionX,
                     foldingPoint = foldingX)
  cmm = createMatrix(NN = Nx, nsd = neis_sdX)
  # print("Initialized!")
  ...
  return(tibble(seed = seedX, reinforce = reinforceX,
                ...
                foldingPoint = foldingX, neis_sd = neis_sdX,
...
for (seed in 1:50) {
  ...
  results = tibble(seed = NA_integer_, reinforce = NA,
                   ...
                   foldingPoint = NA_real_, neis_sd = NA_real_,
  ...
  for (communicationRate in c(.15, .35, .55, .75, .95)) {
    for (forgeting in c(.2, .45, .7, .95)) {
      for (foldingPoint in c(.15, .35, .55, .75, .95)) {
        for (neis_sd in c(0, 0.6, 1.2, 2.4, 3.6)) {
              results = results %>%
                add_row(
                  experiment(
                    seedX = seed, reinforceX = reinforce,
                    communicationRateX = communicationRate,
                    forgetingX = forgeting, neis_sdX = neis_sd,
              ...
              print(
                paste0("Simulation ", SIM," just done, time elapsed: ",
                       ...
                       ", comm.rate=", communicationRate, ", neis_sd=", neis_sd,
                       ", rounds=100).")
              )
              SIM = SIM + 1
        }
...
# Saving ------------------------------------------------------------------
  save(results, file = paste0("results5_seeds_", seed, ".RData"))
}
