#### Main script for project with Guga on folded opinion, EXP10
#### In this version we play with 'Black Pete' scenario:
####   we reintroduce latitude of acceptance once again.

## Encoding: windows-1250
## Created:  2023-06-29 Fran»esko
## Edited:   2023-08-11 Fran»esko
...
for (seed in c(1:400)) {
  ...
  results = tibble(seed = NA_integer_, opDistribution = NA_character_, reinforce = NA,
                   ...
                   b_fi_1 = x5[1], b_fi_2 = x5[2], b_fi_3 = x5[3], b_fi_4 = x5[4], b_fi_5 = x5[5], b_fi_6 = x5[6],
                   b_fi_7 = x5[7], b_fi_8 = x5[8], b_fi_9 = x5[9], b_fi_10 = x5[10], b_fi_11 = x5[11])
  for (communicationRate in c(.35, .65)) {
    for (forgeting in c(.45, .7, .95)) {
      for (foldingPoint in c(.05, .35, .65)) {
        for (meanWeight in c(0.5, 1.5)) {
          for (acceptanceAv in c(0.2, 0.7, 1.2)) {
            for (acceptanceSD in c(0, 0.2)) {
            results = results %>%
                add_row(
                  experiment(
...
# Saving ------------------------------------------------------------------
  save(results, file = paste0("results10_seeds_", seed, ".RData"))
}
