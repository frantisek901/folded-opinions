#### Main script for project with Guga on folded opinion, EXP7
#### In this version we test a weird idea:
#### Weight of own opinion reinforcement and its variability.

## Encoding: windows-1250
## Created:  2023-06-29 Fran»esko
## Edited:   2023-08-02 Fran»esko
...
createPublic =
  function(opDistribution = "Black Pete", N = 1000,
           foldingPoint = 0.05, meanWeight = 1, sdFractionWeight = 0.4,
           minWeight = 0.05, attentionDenom = sqrt(2)) {
  # Needed constants
...
experiment =
  ...
  agm = createPublic(N = Nx, opDistribution = opDistributionX, meanWeight = meanWeightX,
                     sdFractionWeight = sdFractionWeightX, foldingPoint = foldingX)
  ...
  return(tibble(seed = seedX, reinforce = reinforceX,
                ...
                foldingPoint = foldingX, meanWeight = meanWeightX,
...
for (seed in 1:50) {
  ...
  results = tibble(seed = NA_integer_, reinforce = NA,
                   ...
                   foldingPoint = NA_real_, meanWeight = NA_real_,
                   sdFractionWeight = NA_real_,
                   SD = NA_real_, manhattan = NA_real_,
                   ESBG = NA_real_, opDistribution = NA_character_)
  for (communicationRate in c(.15, .35, .55, .75, .95)) {
    for (forgeting in c(.2, .45, .7, .95)) {
      for (foldingPoint in c(.15, .35, .55, .75, .95)) {
        for (meanWeight in c(0.5, 1, 1.5)) {
          for (sdFractionWeight in c(0, 0.2, 0.4)) {
            results = results %>%
                add_row(
                  experiment(
              ...
              print(
                paste0("Simulation ", SIM," just done, time elapsed: ",
                       ...
                       ", sdFractionWeight=", sdFractionWeight, ", rounds=100).")
              )
              SIM = SIM + 1
          }
        }
...
# Saving ------------------------------------------------------------------
  save(results, file = paste0("results7_seeds_", seed, ".RData"))
}
