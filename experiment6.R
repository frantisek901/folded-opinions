...
experiment =
  function(opDistributionX = "Black Pete", rounds = 100, Nx = 1000, seedX = 1,
           ...
           foldingX = foldingPoint,  neisX = neis,
  ...
  cmm = createMatrix(NN = Nx, neis = neisX)
  # print("Initialized!")
  ...
  return(tibble(seed = seedX, reinforce = reinforceX,
                ...
                foldingPoint = foldingX, neis = neisX,
...
for (seed in 1:50) {
  ...
  results = tibble(seed = NA_integer_, reinforce = NA,
                   ...
                   foldingPoint = NA_real_, neis = NA_real_,
  ...
  for (communicationRate in c(.15, .35, .55, .75, .95)) {
    for (forgeting in c(.2, .45, .7, .95)) {
      for (foldingPoint in c(.15, .35, .55, .75, .95)) {
        for (neis in c(2.1, 4.1, 6.1, 8.1, 10.1)) {
              results = results %>%
                add_row(
                  experiment(
                    seedX = seed, reinforceX = reinforce,
                    communicationRateX = communicationRate,
                    forgetingX = forgeting, neisX = neis,
              ...
              print(
                paste0("Simulation ", SIM," just done, time elapsed: ",
                       ...
                       ", comm.rate=", communicationRate, ", neis=", neis,
                       ", rounds=100).")
              )
              SIM = SIM + 1
        }
...
# Saving ------------------------------------------------------------------
  save(results, file = paste0("results6_seeds_", seed, ".RData"))
}
