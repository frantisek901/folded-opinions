#### Main script for project with Guga on folded opinion, EXP13
#### In this version we play with 'Black Pete' scenario:
####   Still same experiment just trying get more smoothness,
####   one more parameter (storing ratio) and recording agents opinions.

## Encoding: windows-1250
## Created:  2023-06-29 Fran»esko
## Edited:   2023-08-17 Fran»esko
...
#   pia = fraction of agents selected for initializing communication
  simRound = function(agm = agm, cmm = cmm, forgeting = 0.5, pia = 0.5, bootstrap = T,
                      storing = storingX, reinforce = T, foldingPoint = foldingX) {
    ...
    for (i in ia) {
      ...
      ## if() function. So double-czech that 'buyingOpinion = T':
        ...
        if (abs(a1["opinion"] - a2["opinion"]) <= a1["acceptance"]) {
          if (a2["opinion"] > 0) {
            agm[i, "infoPos"] = agm[i, "infoPos"] + (storing * a2["opinion"])
            if (agm[i, "infoPos"] > 1) agm[i, "infoPos"] = 1
          } else {
            agm[i, "infoNeg"] = agm[i, "infoNeg"] - (storing * a2["opinion"])
        ...
        if (reinforce) {
          if (a1["opinion"] >= 0) {
            ...
            agm[i, "infoPos"] = agm[i, "infoPos"] + (storing * a1["opinion"] * a1["ownOpWeight"])
            if (agm[i, "infoPos"] > 1) agm[i, "infoPos"] = 1  # Cut-off, of course!
          } else {
            # If agent 'i' has negative opinion, adds it to negative silo:
            agm[i, "infoNeg"] = agm[i, "infoNeg"] - (storing * a1["opinion"] * a1["ownOpWeight"])
        ...
        if (abs(a2["opinion"] - a1["opinion"]) <= a2["acceptance"]) {
          if (a1["opinion"] > 0) {
            agm[p, "infoPos"] = agm[p, "infoPos"] + (storing * a1["opinion"])
            if (agm[p, "infoPos"] > 1) agm[p, "infoPos"] = 1
          } else {
            agm[p, "infoNeg"] = agm[p, "infoNeg"] - (storing * a1["opinion"])
        ...
        if (reinforce) {
          if (a2["opinion"] >= 0) {
            ...
            agm[p, "infoPos"] = agm[p, "infoPos"] + (storing * a2["opinion"] * a2["ownOpWeight"])
            if (agm[p, "infoPos"] > 1) agm[p, "infoPos"] = 1  # Cut-off, of course!
          } else {
            # If agent 'ip' has negative opinion, adds it to negative silo:
            agm[p, "infoNeg"] = agm[p, "infoNeg"] - (storing * a2["opinion"] * a2["ownOpWeight"])
            if (agm[p, "infoNeg"] > 1) agm[p, "infoNeg"] = 1  # Cut-off, of course!
          }
        }
      ## Discussing agents immediately update:
      #          attention and
      agm[c(i, p), "attention"] = (agm[c(i, p), "infoPos"] + agm[c(i, p), "infoNeg"]) / attentionDenom
      agm[agm[, "attention"] > 1, "attention"] = 1

      #          opinion of all agents
      agm[c(i, p), "opinion"] = -agm[c(i, p), "opinion"] ^ 3 + (agm[c(i, p), "infoPos"] + agm[c(i, p), "infoNeg"] - foldingPoint) * (agm[c(i, p), "opinion"]) + (agm[c(i, p), "infoPos"] - agm[c(i, p), "infoNeg"])
...
experiment =
  ...
  ## Simulation including storing:
  #     selecting sample of agents for recording their development:
  selectedAgents = c(sample(1:900, 45), sample(901:1000, 5))

  #     recording initial state:
  record = tibble(as_tibble(agm[selectedAgents, c("ID", "infoPos", "infoNeg", "attention", "opinion")]),
                  round = 0, seed = seedX, communicationRate = communicationRateX,
                  forgeting = forgetingX, foldingPoint = foldingX, storing = storingX,
                  meanWeight = meanWeightX, acceptanceAv = acceptanceAvX)
  ...
  for (r in 1:rounds) {
    agm = simRound(agm = agm, cmm = cmm, forgeting = forgetingX, foldingPoint = foldingX,
      ...
      #     recording simmulation step:
      record = record %>%
        add_row(
          tibble(
            as_tibble(agm[selectedAgents, c("ID", "infoPos", "infoNeg", "attention", "opinion")]),
            round = r, seed = seedX, communicationRate = communicationRateX,
            forgeting = forgetingX, foldingPoint = foldingX, storing = storingX,
            meanWeight = meanWeightX, acceptanceAv = acceptanceAvX))
  }
  #     saving record as data:
  save(record, file = paste0("record13_seeds_", xStemSeed, "_sim_", xSeed, ".RData"))
  ...
  print(paste0("Simulation ", SIMx," done, time: ",
           ...
           ", storing=", storingX, ", forgeting=", forgetingX, ", folding=", foldingX,
  ...
  return(tibble(seed = seedX, opDistribution = opDistributionX, reinforce = reinforceX,
                ...
                foldingPoint = foldingX, storing = storingX,
                meanWeight = meanWeightX, sdFractionWeight = sdFractionWeightX,
...
# Saving ------------------------------------------------------------------
  save(results, file = paste0("results13_seeds_", stemSeed, ".RData"))
}
